...
TARG.Name:=runtime-provider-aliyun
TRAG.Gopkg:=openpitrix.io/runtime-provider-aliyun
...
.PHONY: build
build: fmt ## Build all runtime-provider-aliyun images
	mkdir -p ./tmp/bin
...
.PHONY: compose-up
compose-up: ## Launch runtime-provider-aliyun in docker compose
	docker-compose up -d
...
build-image-%: ## build docker image
	@if [ "$*" = "latest" ];then \
	docker build -t openpitrix/runtime-provider-aliyun:latest .; \
	elif [ "`echo "$*" | grep -E "^v[0-9]+\.[0-9]+\.[0-9]+"`" != "" ];then \
	docker build -t openpitrix/runtime-provider-aliyun:$* .; \
	fi

push-image-%: ## push docker image
	@if [ "$*" = "latest" ];then \
	docker push openpitrix/runtime-provider-aliyun:latest; \
	elif [ "`echo "$*" | grep -E "^v[0-9]+\.[0-9]+\.[0-9]+"`" != "" ];then \
	docker push openpitrix/runtime-provider-aliyun:$*; \
	fi